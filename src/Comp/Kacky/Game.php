...
class Game {
   ...
   *
   * @param Player $player	playing the action card
   * @param ActionCard $cardInHand 	to be played
   * @param array $params 	holds the information about the target duck and action card parameters needed to play
   * @return bool
   */
      ...
      case ActionCard::ZIVY_STIT:
        ...
        if ($secondCard === null) {
          ...
          if ($cardTable == 0) {
            ...
            if (($this->is_mine(Player::COLOR_WATER, $cardTable - 1) || $this->is_mine($myColor, $cardTable - 1) || $this->is_duck_on_duck($cardTable - 1)) &&
                ($this->is_mine(Player::COLOR_WATER, $cardTable + 1) || $this->is_mine($myColor, $cardTable + 1) || $this->is_duck_on_duck($cardTable + 1))) {
              return false;
            }
          }
  ...
  private function get_possible_moves($player_id) {
    ...
    foreach ($player->getHand() as $k => $cardInHand) {
      switch($cardInHand->getClass()) {
        case ActionCard::CLASS_DOUBLE: // JEJDA_VEDLE, ZIVY_STIT
          $moves[$k] = [];
        ...
        case ActionCard::CLASS_SPECIAL: // ROSAMBO
          $moves[$k] = true;
          break;

        case ActionCard::CLASS_SINGLE:
